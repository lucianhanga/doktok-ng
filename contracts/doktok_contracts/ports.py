...
class TagRepository(Protocol):
    ...
    def merge_into(self, tenant_id: str, loser_id: str, survivor_id: str) -> int:
        """Merge one tag into another (#550): repoint the loser's document links to the survivor
        (de-duped on the PK), mark the loser status='merged' with merged_into=survivor, and
        return how many documents were re-tagged."""
        ...
...
class TenantRegistry(Protocol):
    ...
    def list_tenants(self) -> list[Tenant]:
        """All tenants, ordered by id (admin listing, #559)."""
        ...

    def create_user(self, user: User) -> None: ...
    def get_user(self, tenant_id: str, user_id: str) -> User | None: ...
    def list_users(self, tenant_id: str) -> list[User]:
        """All users in a tenant, ordered by email. Never includes ``password_hash`` (#559)."""
        ...

    def get_user_by_email(self, tenant_id: str, email: str) -> User | None:
        """The user with ``email`` in ``tenant_id``, INCLUDING ``password_hash`` (login, #555).

        Distinct from ``get_user``: this is the only method that returns the credential digest, and
        only the login flow calls it. Returns ``None`` when no such user exists. Email match is
        case-insensitive (addresses are stored lowercased).
        """
        ...

    def set_user_password(self, tenant_id: str, user_id: str, password_hash: str) -> None:
        """Set (or replace) a user's password digest (a scrypt digest, #555)."""
        ...
    ...
    def list_api_tokens(self, tenant_id: str) -> list[ApiToken]:
        """All API tokens in a tenant, newest first. ``token_sha256`` is the stored hash, never the
        plaintext (which exists only at issue time); callers must not return it to clients, #559."""
        ...
