...
- [ ] If password login is enabled: a dedicated `DOKTOK_AUTH_JWT_SECRET` set (do not lean on the
      `DOKTOK_SECRETS_KEY` fallback in production - rotating sessions should not re-key stored
      secrets); admins minimized; unused invitations expired or their users removed (ADR-0024).
...
- **Enabling password login (opt-in).** Set `DOKTOK_AUTH_JWT_SECRET` (long and random; without it,
  `DOKTOK_SECRETS_KEY` is used as the fallback signing secret, and with neither, login is disabled
  with a 503). Session JWTs live `DOKTOK_AUTH_ACCESS_TTL_SECONDS` (default 3600) - keep the TTL
  modest, since a session cannot be individually revoked before expiry.
...
- **Invitations** expire after `DOKTOK_AUTH_INVITE_TTL_HOURS` (default 168). The invite token is a
  ...
  authenticate.
- **Audit**: administration and membership events (role changes, password resets, token
  issue/revoke, invites, deactivations) are recorded in the activity log with the acting user (or
  tenant, for the login-less operator) as the actor.
